...
import icl.ohs.reference.feature.web.WebContentScreen
...
private val SET_NEW_PASSWORD_SCREEN_CONFIG =
  SetNewPasswordScreenConfig(endpoint = ApiConstants.RESET_PASSWORD_ENDPOINT, showFooter = true)
private val RESET_PASSWORD_SCREEN_CONFIG =
  ResetPasswordScreenConfig(endpoint = ApiConstants.RESET_PASSWORD_ENDPOINT, showFooter = true)

private val AUTH_WEB_LINKS =
  AuthWebLinksConfig(
    // Replace ApiConstants.TERMS_AND_CONDITIONS_URL / PRIVACY_POLICY_URL with your deployed
    // legal and policy pages.
    termsAndConditions =
      AppWebLink(
        key = "terms-and-conditions",
        title = "Terms and Conditions",
        url = ApiConstants.TERMS_AND_CONDITIONS_URL,
      ),
    privacyPolicy =
      AppWebLink(
        key = "privacy-policy",
...
private fun NavController.navigateToSetNewPassword(initialIdNumber: String) {
  navigate("$AUTH_SET_NEW_PASSWORD_ROUTE?$SET_NEW_PASSWORD_ID_NUMBER_ARG=${initialIdNumber.trim()}")
}

private fun NavController.navigateToResetPassword(identifier: String) {
  navigate("$AUTH_RESET_PASSWORD_ROUTE?$RESET_PASSWORD_IDENTIFIER_ARG=${identifier.trim()}")
}

private fun NavController.navigateToForgotPassword(initialIdentifier: String) {
  navigate(
    "$AUTH_FORGOT_PASSWORD_ROUTE?$FORGOT_PASSWORD_IDENTIFIER_ARG=${initialIdentifier.trim()}"
  )
}

private fun NavController.navigateToWebLink(link: AppWebLink) {
